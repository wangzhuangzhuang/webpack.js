	...
	init: {
		PostInit: function() {
			document.getElementById('saveBtn').onclick = function() {
				...
				if(!picture.NameFn($("#add_name"))) return false;
				if(!picture.identificationFN($("#add_number"))) return false;
               
                var ls = $(".upload_imgs .inputBox img").length;
                if(ls == 0){
                	order.saveFn("","");
                }else if(ls == 1){
                	$("#prompt").show();
 			        $("#prompt").text("请选择两张身份证");
                   
                }else if($("#wzadd_add_idcard").attr("data-k") == 1){
                	var img1 = $("#sec1").find("img").attr("src");
                	var img2 = $("#sec2").find("img").attr("src");
                	order.saveFn(img1,img2);
                }else{
		...
		FilesAdded: function(up, files) {
		   ...
		   if($("#wzadd_add_idcard").attr("data-k") == 1){
		   	   $("#wzadd_add_idcard").attr("data-k","2")
		   	   $("#sec2").html("");
		   };
